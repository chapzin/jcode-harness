...
| --- | --- | --- | --- |
| Offline built-in skills | Done | `src/skill_pack.rs` embeds `karpathy-guidelines`, `optimization`, `clean-code-guardian`, `llmwiki-memory`, `init-bootstrap`, and `sequential-thinking` with `include_str!`; unit tests assert all built-ins parse; e2e coverage asserts `skills doctor` duplicate reporting. | Keep duplicate reporting deterministic as new origins are added. |
...
| Deterministic skill router | Done | `src/skill_router.rs` supports `auto`, `off`, `always`, explicit skills, coding terms, perf terms, LLM wiki/project-memory terms, init/bootstrap terms, and sequential-thinking terms, with unit and CLI dry-run coverage for proposal guarantees. | Keep trigger vocabulary conservative and test every expansion. |
...
| Init and sequential thinking skills | Done | `init-bootstrap` documents `/init`, `.context`/`.jcode` scaffold upkeep, swarm init analysis, side panels, skills plans, and MCP plans; `sequential-thinking` documents bounded use of the local sequential-thinking MCP helper; router and e2e coverage assert combined `skills match` selection. | Keep MCP activation review-first and do not auto-enable servers in `.jcode/mcp.json`. |
